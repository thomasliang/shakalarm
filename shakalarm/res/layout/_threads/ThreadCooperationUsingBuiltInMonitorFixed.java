...
public class ThreadCooperationUsingBuiltInMonitorFixed {
	...
	private static class Account {
		...
		public synchronized void withdraw(int amount) {
			try {
				int triedDepositID = 0;
				while (balance < amount) {
					System.out.println("\t\t\tWithdraw " + amount
							+ " pending... \t" + getBalance());
					if (balance > 0) notify();
					triedDepositID = this.newDepositID;
					while (triedDepositID == this.newDepositID) wait();
				}
				balance -= amount;
				if (balance > 0) notify();
				System.out.println("\t\t\tWithdraw " + amount + "\t\t"
		...
		public synchronized void deposit(int amount) {
			...
			notify();
			++newDepositID;	// may increment it using a mod operator
		}
		private int newDepositID;
	}
}
